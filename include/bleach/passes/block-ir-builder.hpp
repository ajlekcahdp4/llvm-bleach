...
public:
  block_ir_builder_pass(const instr_impl &insts, bool functions_nop,
                        std::string_view state_file, const mctomir::file_info *file_info, unsigned stack_sz)
      : instrs(insts), state_struct_file(state_file), finfo(file_info), stack_size(stack_sz),
