...
int main() {
  ...
  arguments arg_array[] = {
      ...
      {1477171087, 356426808, 945117276, 1889947178, 3824015087501868208, 1055929314494252795, 1615751137563298991, 4410931268227206996, 31.460815, 0.000072, -12.088020, 84.793961, -1.601563, -1.185233, -1.315781, -1.089028},
      {1477171087, 356426808, 945117276, 1889947178, 3824015087501868208, 1055929314494252795, 1615751137563298991, 4410931268227206996, 31.460815, 0.000072, -12.088020, 84.793961, -1.601563, -1.185233, -1.315781, -1.089028},
  ...
  for (unsigned i = 0; i < sizeof(arg_array) / sizeof(arguments); ++i) {
    ...
    failed |= res;
    printf("ARGUMENTS WERE:\n %d, %d, %u, %u, %ld, %ld, %lu, %lu, %f, %f, %f, %f, %lf, %lf, %lf, %lf\n", args->i1, args->i2, args->u1, args->u2, args->l1, args->l2, args->ul1, args->ul2, args->f1, args->f2, args->f3, args->f4, args->d1, args->d2, args->d3, args->d4);
      for (int i = 0; i< 32; ++i)
        printf("GPR[%d] = %lx\n", i, regs.GPR[i]);
      for (int i = 0; i< 32; ++i)
        printf("FPR[%d] = %lx\n", i, regs.FPR[i]);
