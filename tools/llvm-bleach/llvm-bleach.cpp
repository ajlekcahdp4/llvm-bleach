...
static cl::opt<std::string>
  symtab_file("symtab-file", cl::desc("Optional symtab file. Needed for lifting indirecy branches."), cl::cat(options));
...
auto main(int argc, char **argv) -> int try {
  ...
  if (symtab_file.getNumOccurrences()) {
    ...
    finfo = std::make_unique<mctomir::file_info>(mctomir::load_file_info_from_yaml(symtab_contents));
  }
  ...
  mpm.addPass(bleach::lifter::block_ir_builder_pass(
      instrs, assume_function_nop.getValue(), dump_struct_def_option.getValue(), finfo.get(),
